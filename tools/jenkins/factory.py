...
class EnhancedJenkinsClient:
    ...
    async def async_get_build_info(
        self, job_name: str, build_number: int
    ) -> Dict[str, Any]:
        return await self._async_request(
            "GET", f"/job/{job_name}/{build_number}/api/json"
        )
    ...
    async def async_get_console_output(self, job_name: str, build_number: int) -> str:
        response = await self._async_request(
            "GET", f"/job/{job_name}/{build_number}/consoleText"
        )
        return response.get("content", "Could not retrieve console output.")

    def trigger_build(
        self, job_name: str, parameters: Optional[Dict[str, str]] = None
    ) -> Dict[str, Any]:
    ...
    def wait_for_build_completion(
        self, job_name: str, build_number: int, timeout: int = 300
    ) -> Dict[str, Any]:
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                return {
                    "status": "ERROR",
                    "error": f"Failed to get build info during wait: {e}",
                }
        return {"status": "TIMEOUT", "error": "Timeout waiting for build completion"}

    async def async_wait_for_build_completion(
        self, job_name: str, build_number: int, timeout: int = 300
    ) -> Dict[str, Any]:
    ...
    async def async_get_pipeline_health(
        ...
        async def get_pipeline_info(name: str):
            try:
                ...
                return (
                    name,
                    {"status": status, "last_build_number": last_build.get("number")},
                    status not in ["SUCCESS", "UNSTABLE"],
                )
            except Exception:
                return (
                    name,
                    {"status": "NOT_FOUND", "error": "Could not retrieve job info."},
                    True,
                )

        results = await asyncio.gather(
            *[get_pipeline_info(name) for name in pipeline_names],
...
class TriggerBuildInput(ToolInputSchema):
    ...
    parameters: Optional[Dict[str, str]] = Field(
        default=None, description="Build parameters as key-value pairs."
    )


class JobStatusInput(ToolInputSchema):
    job_name: str = Field(
        description="Name of the Jenkins job/pipeline to check the status of its last build."
    )
...
class BuildInfoInput(ToolInputSchema):
    ...
    build_number: int = Field(
        description="The specific build number to get information for."
    )
...
class ConsoleOutputInput(ToolInputSchema):
    job_name: str = Field(description="Name of the Jenkins job/pipeline.")
    build_number: int = Field(
        description="The build number to get the console output for."
    )
    tail_lines: Optional[int] = Field(
        default=100, description="Number of lines from the end of the log to return."
    )
...
class PipelineMonitorInput(ToolInputSchema):
    ...
    timeout: int = Field(
        default=600, description="Timeout in seconds to wait for the build to complete."
    )
...
class RollbackInput(ToolInputSchema):
    ...
    environment: str = Field(
        description="The target environment for the rollback (e.g., 'production', 'staging')."
    )
    version_to_restore: str = Field(
        description="The specific application version to restore."
    )


class EmergencyDeployInput(ToolInputSchema):
    job_name: str = Field(description="Name of the emergency deployment job.")
    branch_or_commit: str = Field(
        description="The Git branch or commit hash for the emergency deploy."
    )
    environment: str = Field(
        description="The target environment for deployment (e.g., 'production', 'staging')."
    )


class HealthCheckInput(ToolInputSchema):
    pipeline_names: List[str] = Field(
        description="A list of critical pipeline names to include in the health check."
    )
...
class JenkinsTriggerBuildTool(AbstractTool):
    ...
    description: str = (
        "Triggers a Jenkins build for a specified job, optionally with parameters."
    )
...
class JenkinsBuildInfoTool(AbstractTool):
    ...
    description: str = (
        "Gets detailed information for a specific build number of a Jenkins job."
    )
    ...
    async def _arun(self, job_name: str, build_number: int) -> str:
        try:
            info = await self.jenkins_client.async_get_build_info(
...
class JenkinsConsoleOutputTool(AbstractTool):
    ...
    description: str = (
        "Retrieves the console log for a specific Jenkins build. Can tail the log."
    )
    ...
    async def _arun(
        ...
        try:
            output = await self.jenkins_client.async_get_console_output(
                job_name, build_number
            )
            log_lines = output.splitlines()
            tailed_output = "\n".join(log_lines[-tail_lines:])
...
class JenkinsGetLastBuildInfoTool(AbstractTool):
    ...
    description: str = (
        "Retrieves detailed information about the most recent build of a Jenkins job."
    )
    args_schema: Type[ToolInputSchema] = (
        JobStatusInput  # Re-using JobStatusInput is fine here
    )
    jenkins_client: EnhancedJenkinsClient
...
class JenkinsPipelineMonitorTool(AbstractTool):
    ...
    description: str = (
        "Monitors a specific Jenkins build until it completes or times out."
    )
    ...
    def _run(self, job_name: str, build_number: int, timeout: int = 600) -> str:
        try:
            result = self.jenkins_client.wait_for_build_completion(
                job_name, build_number, timeout
            )
    ...
    async def _arun(self, job_name: str, build_number: int, timeout: int = 600) -> str:
        try:
            result = await self.jenkins_client.async_wait_for_build_completion(
                job_name, build_number, timeout
            )
            return json.dumps({"success": True, "monitoring_result": result})
        except Exception as e:
...
class JenkinsEmergencyDeployTool(AbstractTool):
    ...
    description: str = (
        "Triggers an emergency deployment pipeline with a specific branch or commit."
    )
    ...
    def _run(self, job_name: str, environment: str, branch_or_commit: str) -> str:
        ...
        try:
            ...
            return json.dumps(
                {
                    "success": True,
                    "message": "Emergency deploy triggered.",
                    "details": result,
                }
            )
        except Exception as e:
    ...
    async def _arun(
        self, job_name: str, environment: str, branch_or_commit: str
    ) -> str:
        parameters = {"ENVIRONMENT": environment, "GIT_REF": branch_or_commit}
        try:
            result = await self.jenkins_client.async_trigger_build(job_name, parameters)
...
class JenkinsRollbackTool(AbstractTool):
    ...
    description: str = (
        "Triggers a rollback pipeline to restore a previous version in an environment."
    )
    ...
    def _run(self, job_name: str, environment: str, version_to_restore: str) -> str:
        ...
        try:
            result = self.jenkins_client.trigger_build(job_name, parameters)
            return json.dumps(
                {"success": True, "message": "Rollback triggered.", "details": result}
            )
        except Exception as e:
            return json.dumps({"success": False, "error": str(e)})

    async def _arun(
        self, job_name: str, environment: str, version_to_restore: str
    ) -> str:
        parameters = {
            "ENVIRONMENT": environment,
            "VERSION_TO_RESTORE": version_to_restore,
...
class JenkinsToolFactory:
    ...
    def create_all_tools(self) -> List[AbstractTool]:
        ...
        return [
            tool_class(jenkins_client=self.jenkins_client)
...
if __name__ == "__main__":
    ...
    async def test_async_functionality():
        try:
            ...
            status_tool = next(
                (t for t in all_jenkins_tools if t.name == "jenkins_job_status"), None
            )
            if status_tool:
            ...
            print(
                "Please set JENKINS_URL, JENKINS_USERNAME, and JENKINS_API_TOKEN environment variables in a .env file."
            )
        except Exception as e:

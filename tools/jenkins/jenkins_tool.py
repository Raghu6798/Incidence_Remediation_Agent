...
class JenkinsClient:
    ...
    def trigger_build(
        self, job_name: str, parameters: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
...
class BuildInfoInput(BaseModel):
    ...
    build_number: Optional[int] = Field(
        default=None, description="Build number (if not provided, gets last build)"
    )
    ...
    def _run(
        ...
        **kwargs,
        ...
        try:
            ...
            return json.dumps({"success": False, "error": str(e), "job_name": job_name})
    ...
    def _run(
        ...
        **kwargs,
        ...
        try:
            ...
            return json.dumps({"success": True, "build_info": result_info})
        except Exception as e:
            return json.dumps(
                {
                    "success": False,
...
class JenkinsConsoleOutputTool(AbstractTool):
    ...
    def _run(
        ...
        **kwargs,
        ...
        try:
            console_output = self.jenkins_client.get_console_output(
...
class PipelineStatusInput(BaseModel):
    ...
    wait_for_completion: bool = Field(
        default=False, description="Whether to wait for the build to complete"
    )
    timeout_seconds: int = Field(
        default=300, description="Timeout for waiting (default 5 minutes)"
    )
...
class RollbackInput(BaseModel):
    ...
    target_version: Optional[str] = Field(
        default=None, description="Target version/commit to rollback to"
    )
...
class EmergencyDeployInput(BaseModel):
    ...
    branch_or_commit: str = Field(
        description="Branch or commit hash for emergency deploy"
    )
    environment: str = Field(description="Target environment")
    skip_tests: bool = Field(
        default=False, description="Skip tests for faster deployment"
    )
...
class HealthCheckInput(BaseModel):
    ...
    pipeline_names: List[str] = Field(
        description="List of critical pipeline names to check"
    )
            ...
            try:
                ...
                if last_build:
                    build_info = self.get_build_info(
                        pipeline_name, last_build.get("number")
                    )
                ...
                if last_successful:
                    pipeline_status["last_successful_build"] = last_successful.get(
                        "number"
                    )
